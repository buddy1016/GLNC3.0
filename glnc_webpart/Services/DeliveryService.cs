...
namespace glnc_webpart.Services
{
    public class DeliveryService : IDeliveryService
    {
        ...
        public async Task<int> GetDeliveriesCountAsync()
        {
            return await _context.Deliveries.CountAsync();
        }

        public async Task<List<Delivery>> GetDeliveriesPagedAsync(int offset, int limit)
        {
            return await _context.Deliveries
                .Include(d => d.Truck)
                .OrderByDescending(d => d.DateTimeAppointment)
        ...
        public async Task<List<Delivery>> GetDeliveriesByDateRangeAsync(DateTime startDate, DateTime endDate)
        {
            return await _context.Deliveries
                .Include(d => d.Truck)
                .Include(d => d.User)
                .Where(d => d.DateTimeAppointment >= startDate && d.DateTimeAppointment <= endDate)
                .OrderBy(d => d.DateTimeAppointment)
                .ToListAsync();
        }
